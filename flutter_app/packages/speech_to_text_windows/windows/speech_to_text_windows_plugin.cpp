...
// speech_to_text Dart expects SpeechRecognitionResult JSON (alternates + finalResult).
...
void SpeechToTextWindowsPlugin::SendTextRecognition(const std::string& text, bool is_final) {
  if (m_channel) {
    const std::string esc = JsonEscape(text);
    std::string json_result =
        std::string(
            "{\"alternates\":[{\"recognizedWords\":\"") +
        esc +
        "\",\"recognizedPhrases\":null,\"confidence\":-1}],\"finalResult\":" +
        (is_final ? "true" : "false") + "}";
    std::cout << "Sending to Flutter (fixed JSON shape): " << json_result << std::endl;
